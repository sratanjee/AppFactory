#!/usr/bin/env bash
# Runs wash-quote with --dart-define values sourced from factory.config so
# RevenueCat / PostHog keys land in app_config.dart's String.fromEnvironment
# reads. Pass any extra args through to `flutter run`.
#
# Per-slug key lookup: REVENUECAT_IOS_KEY_wash_quote /
# REVENUECAT_ANDROID_KEY_wash_quote are the canonical names in
